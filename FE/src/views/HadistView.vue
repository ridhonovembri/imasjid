          ...
          <q-card-section class="q-pt-none">
            <q-form class="q-gutter-md">
              <q-list>
                <q-item>
                  <q-item-section>
                    <q-item-label class="q-pb-xs">Isi Hadist</q-item-label>
                    <q-input dense outlined v-model="hadist.HadistContent" />
                  </q-item-section>
                </q-item>
                <q-item>
                  <q-item-section>
                    <q-item-label class="q-pb-xs">Hadist Riwayat</q-item-label>
                    <q-input dense outlined v-model="hadist.HadistFrom" />
                  </q-item-section>
                </q-item>
              </q-list>
            </q-form>
  ...
  export default {
    ...
    methods: {
      getSelectedRow() {
        ...
        this.hadist.Id = this.selected[0].Id
      },
  
      async getRow() {
        this.rows = (await Get.masjidHadist()).data;
        // console.log('rows', this.rows)
      },
  
      editRow(props) {
        if (props) {
          this.hadist.Id = props.row.Id;
          this.hadist.HadistContent = props.row.HadistContent;
          this.hadist.HadistFrom = props.row.HadistFrom;
        }
      ...
      async deleteRow() {
        ...
        const id = this.hadist.Id;
  
        // const index = this.rows.indexOf(id);
        await Delete.masjidHadist(id).then((res) => {
      ...
      async updateRow() {
        ...
        let id = this.hadist.Id;
        // console.log("id", id);
        // console.log('role', this.role)
  
        if (id > 0) {
          await Update.masjidHadist(id, this.hadist).then((res) => {
            // console.log('res', res)
          ...
          let data = {
            HadistContent: this.hadist.HadistContent,
            HadistFrom: this.hadist.HadistFrom,
          };
  
        //   console.log('data', data)
  
          await Post.masjidHadist(data).then((res) => {
            const status = res.data.status;
            const message = res.data.message;
            // console.log("after update", status);
      ...
      showForm() {
        ...
        this.hadist.Id = "";
        this.hadist.HadistContent = "";
        this.hadist.HadistFrom = "";
        this.show_form = true;
      },
